...
class AddBlock extends React.Component{
  constructor() {
  super();
  this.addCategoryUrl= 'http://localhost:4209/block/addblock';
  this.getCategoryUrl= 'http://localhost:4209/block/getblock';
  this.delCategoryUrl='http://localhost:4209/block/delblock';
  ...
  getList(){
    const user_id =localStorage.getItem('user_id');
     ...
     axios.post(this.getCategoryUrl,formData)
      .then(data => {
              if(data.data.code==200){
                this.initialblockList = data.data.result.map((values) => {
                    return values
                });
              setTimeout(function(){
                  this.setState({
  ...
  render() {
    ...
    return(<div className="row" style={{'font-size':'12px'}}>
      <SweetAlert
        ...
        />
          ...
          <div className="card-header"><b>Add New Block</b></div>
          <div class="card-body" style={{'font-size':'12px'}}>
            <form onSubmit={this.handleSubmit} className="form" style={{'margin':5}} ref={(el) => this.myFormRef = el}>
            <div class="form-group">
            <label>Enter Category Name</label>
      <input type="text" name="title" class="form-control" placeholder="Enter Block Title Here" maxlength={100} style={{marginBottom: 2}} 
      ...
      <button type="submit" class="btn btn-success btn-sm">Cancel</button>
          ...
          <div class="row" style={{'font-size':'12px'}}>
            ...
            {(loading==false)?(<FadeIn><BlockList onClick={this.updateCategory} state={this.state}/></FadeIn>):(<center><img src={Loader}/></center>)}
            </div>
            </div>
          </div>);  
  }
