...
ArticleRouter.route('/update').post(function (req, res) {
  var id = req.body.id;
  ...
  var tagInput = req.body.tagInput;
  var status = req.body.status;
  var result =null;
  var result = stringify(tagInput);
  ...
  db.connection.getConnection(function(err,connection){
    ...
    if(result!=''){
      ...
      var sql = "UPDATE posts SET title = '"+title+"', description = '"+description+"', category_id = '"+category_id+"', status = '"+status+"', tags = '"+result+"' where id = "+id;
      var values  = [title, description, category_id, status, result, id];
    }else{
      var sql = "UPDATE posts SET title = ?, description = ?, category_id = ?, status = ? where id = ?";
      var values  = [title, description, category_id, status, id];
    }
    ...
    connection.query(sql, function(error, results) {
          if (error) throw res.json(error);
            ...
            res.json({status:'success',message:'Post updated Successfully',code:'200'});
          })
...
ArticleRouter.route('/add').post(function (req, res) {
  var description = req.body.description;
  var title = req.body.title;
  var category_id = req.body.categoryId;
  var user_id = req.body.user_id;
  ...
  db.connection.getConnection(function(err,connection){
    ...
    if(result!=''){
       console.log('pok==='+result);
      var sql = "INSERT INTO posts (title,description,user_id,category_id,tags) VALUES ?";
      var values = [[title,description,user_id,category_id,result]];
    }else{
      var sql = "INSERT INTO posts (title,description,user_id,category_id) VALUES ?";
      var values = [[title,description,user_id,category_id]];
    }
...
ArticleRouter.route('/').get(function (req, res) {
    
});

ArticleRouter.route('/getarticlebyid').post(function (req, res) {
  var id = req.body.id;
  var token       = req.body.token;
  var sql = "SELECT c.title as categoryName ,p.* from posts p  LEFT JOIN category c  on p.category_id = c.id where p.id=?";
  console.log(sql);
  var values = [[id]];
  db.connection.query(sql,[values],function(error, rows,fields) {
            if (error) throw res.json(error)
              numRows = rows.length;
              if(numRows==0){
...
ArticleRouter.route('/getuserpost').post(function (req, res) {
  var user_id      = req.body.user_id;
  var token       = req.body.token;
  if(sha1(config.salt_1 + user_id)==token){
    db.connection.getConnection(function(err,connection){
        ...
        console.log('You are now connected... id ' + connection.threadId);
        var sql = "SELECT u.first_name,u.last_name,p.*,c.id as category_id, c.title as categoryName from posts p JOIN users u on p.user_id=u.id  LEFT JOIN category c on c.id=p.category_id order by p.id DESC";
...
ArticleRouter.route('/login').post(function (req, res,next) {
    var sess = {token: '',user: {}};
...
ArticleRouter.route('/register').post(function (req, res) {
