/*
 * @PageName    :: CategoryRouter.js
 * @Author      :: Pradeep Kumar
 * @Description :: All routing of category module
...
CategoryRouter.route('/delcategory').post(function (req, res) {
  var id = req.body.id;
...
CategoryRouter.route('/getcategory').post(function (req, res) {
  var user_id      = req.body.user_id;
  var token       = req.body.token;
...
CategoryRouter.route('/getcategoryoptions').post(function (req, res) {
  var user_id      = req.body.user_id;
  var token       = req.body.token;
  if(sha1(config.salt_1 + user_id)==token){
    db.connection.getConnection(function(err,connection){
        ...
        //Database Connected Now
        console.log('You are now connected... id ' + connection.threadId);
        var sql = "SELECT c.id as value,c.title as label from category c order by c.id DESC";
          connection.query(sql,function(error, rows,fields) {
              if (error) throw res.json(error)
                numRows = rows.length;
                if(numRows==0){
